//---------------------------------------------------【AJAX载入歌曲信息】
$.getJSON(	"/wp-content/themes/twentyfourteen/public/data/songData.json").done(function(data){
    //发送歌曲的信息
    formatPlayer(data);
});
...
function formatPlayer(song){
    var  mEngine = new MUSICENGINE(song),
	        mPlay = $("#musicEngine"),
	        mPlayerSwitch = $("#musicPlayerSwitch"),
	        play = $("#play"),
	        pause = $("#pause"),
	        next = $("#next"),
	        prev = $("#prev"),
	        ...
	        volume = $("#volume"),
	        mute = $("#mute"),
	        volumeSizeBg = $("#volumeSizeBg"),
	        progressRateBg = $("#progressRateBg"),
	        lyrics = $("#lyrics"),
    ...
    $("#musicPlayerWrap").on("click","a",function(){
    	if(this.id === 'play' ){
    		mEngine.toPlay("play");			  //播放
    	}else if(this.id === 'pause'){
    		mEngine.toPlay("pause");		 //暂停
    	...
    	}else if(this.id === 'nowVolume' || 'volume'){		//静音开关
    		 mEngine.volume("on");
		}else if(this.id === 'nowMute' || 'mute'){
			mEngine.volume("off");
		}else if(this.id === 'volumeSizeBg'){
			var realVolume = VolumeChangeProcess(event);
	        mEngine.volumeControlStrip(realVolume);	    //音量控制杆
    	}else if(this.id === 'progressRateBg'){
            if(mPlay.src != ""){												//歌曲进度调节
    	...
    	}else if(this.id === 'albumLists'){		
    		mEngine.albumLists();								//专辑列表
		}else if(this.id === 'closeList'){		
			closeList();
            return false;												//关闭列表
		}else if(this.id === 'cutoverList'){					//返回列表，全部歌曲列表
            if($(this).text() === "返回"){
...
function MUSICENGINE(song){
    //创建播放器
    var musicEngine =  $("<audio id='musicEngine'>");
    $("#musicPlayerWrap").append(musicEngine);
  
    //获取歌曲
    this.song = song;
    //获取播放器
    this.musicPlayer = $("#musicEngine");
    //载入完成
    this.musicPlayer.on("canplaythrough",function(){
        //歌词
        $("#loadLrc").hide();
    });
}
...
MUSICENGINE.prototype.volume = function(muteSwitch){
    var volumeSizeColor = $("#volumeSizeColor"),
        volumeSizeSave = parseInt(volumeSizeColor.height());

    //设为静音
    if(muteSwitch === "on"){
        //记录静音前的音量大小
        volumeSizeColor.attr({"data-volume": this.musicPlayer.volume})
        //记录静音前的元素高度
        volumeSizeColor.attr({"data-height": volumeSizeSave}); 
...
function volumeIconSwitch(muteSwitch){
    var nowVolume = $("#nowVolume"),
        nowMute = $("#nowMute"),
        volume = $("#volume"),
        mute = $("#mute"),
        volumeSizeColor = $("#volumeSizeColor");
    //打开静音
    if(muteSwitch === "on"){
        //静音的显示图标变化
    	nowVolume.hide();
    	...
    	mute.show();
        //音量为0时，让音量杆也为0，优化视觉
        volumeSizeColor[0].style.height = "100%";
    }
    //关闭静音
    if(muteSwitch === "off"){
        //静音的显示图标变化
...
MUSICENGINE.prototype.playbackProgress = function(playSwitch){
    var progressRateColor = $("#progressRateColor"),
        songSchedule = 0,
        timeall,
        currenttime,
        timer;
...
function getSongProgress(event){
    var progressRateBg = $("#progressRateBg"),
        mplayer = $("#musicEngine"),
        progressBP,
        SongProgress;
...
function currentlyPlayingSong(){
    var list = $("#listWrap"),
        indexMusicName = $("#musicName").innerHTML,
        indexAlbumName = $("#albumName").innerHTML;
...
MUSICENGINE.prototype.playIndex = function(albumIndex,songIndex){
    var albumFrontCover = $("#albumFrontCover"),
        ...
        playTime = $("#playTime"),
        surplusTime = $("#surplusTime"),
        progressRateColor = $("#progressRateColor"),
        song = this.song;
...
//---------------------------------------------------【播放方式控件显示&隐藏，调节】
(function(){
    var nowPlayManner = $("#nowPlayManner"),
        playMannerControl = $("#playMannerControl"),
        orderPlay = $("#orderPlay"),
    ...
    playMannerControl.onclick = function(event){
        ...
        PlayModeIconChange($(o.target.id));
    };
})();
...
function playerInitialization(){
    ...
    musicName.text("FeelPlayer");
    musicName[0].title = "FeelPlayer";
    musicSinger.text("播放最爱的音乐");
    musicSinger[0].title = "播放最爱的音乐";
    albumName.text("");
    albumName[0].title = "";
    musicEngine[0].src = "";
    playTime.innerHTML = "0:00";
...
function musicPlayerSwitch(){
    var musicPlayer = $("#musicPlayerWrap"),
        playerSwitch = $("#musicPlayerSwitch"),
        musicList = $("#musicList"),
        timer = 0,
        n = -550,
        m = 0;
    ...
    if(hasClass(playerSwitch,"on")){
        ...
        timer = setInterval(function(){
            n += 50;
            musicPlayer.style.right = n + "px";
            if(musicList){
                musicList.style.right = n + "px";
            }
            if(n >= 0){
                clearInterval(timer);
                //替换类，用于显示播放器打开或隐藏的三角按钮
                replaceClass($("#playerSwitchBtn"),"switch-on","switch-off");
    ...
    //隐藏播放器
        ...
        timer = setInterval(function(){
            m += 50;
            musicPlayer.style.right = "-" + m + "px";
            if(musicList){
                musicList.style.right = "-" + m + "px";
            }
            if(m >= 550){
                clearInterval(timer);
                replaceClass($("#playerSwitchBtn"),"switch-off","switch-on");
...
function lrcBoxSwitch(){
    ...
    //打开
    if(!lyrics.hasClass("lyrics-hover")){
        lyrics[0].title = "关闭歌词";
        lyrics.addClass("lyrics-hover");
        lrcWrap.removeClass("hidden");
    }else{
        //隐藏
    	lyrics[0].title = "打开歌词";
        lyrics.removeClass("lyrics-hover");
        lrcWrap.addClass("hidden");
    }
}
