...
$(function() {
	...
	//the thumb
	var $thumb		= $im_wrapper.children('div');
	//all the images
	var $thumb_imgs = $thumb.find('img');
	//number of images
	var nmb_thumb	= $thumb.length;
	...
	//number of thumb per line
	var per_line	= 6;
	//number of thumb per column
	var per_col		= Math.ceil(nmb_thumb/per_line)
	//index of the current thumb
	var current		= -1;
	//mode = grid | single
	var mode		= 'grid';
	//an array with the positions of the thumb
	//we will use it for the navigation in single mode
	var positionsArray = [];
	for(var i = 0; i < nmb_thumb; ++i)
		positionsArray[i]=i;
		
	//入口
	triggerMerging();
	//disperse();
	function triggerMerging(){
		...
		$thumb_imgs.each(function(){
			...
			$('<img/>').load(function(){
				...
				if(loaded == nmb_thumb*2)
					start();
			}).attr('src',$this.attr('src').replace('/thumb','/mid'));
		});
		
		//starts the animation
		function start(){
			$im_loading.hide();
			//disperse the thumb in a grid
			disperse();
		}
	}
	//disperses the thumb in a grid based on windows dimentions
	function disperse(){
		if(!flg_click) return;
		setflag();
		mode			= 'grid';
		//center point for first thumb along the width of the window
		var spaces_w 	= $(window).width()*1/(per_line + 1);
		//center point for first thumb along the height of the window
		var spaces_h 	= $(window).height()/(per_col + 1);
		//let's disperse the thumb equally on the page
		...
		var im_close_data = $im_wrapper.find("div:nth-child(6)").css(["left","top"]);
		$im_close.css(im_close_data);
	}
	
	//controls if we can click on the thumb or not
	...
	$thumb.bind('click',function(){
		...
		if(mode	== 'grid'){
			...
			var image_src	= $this.find('img').attr('src').replace('/thumb','/mid');
			$thumb.each(function(i){
				var $thumb 	= $(this);
				var $image 	= $thumb.find('img');
				//first we animate the thumb image
				//to fill the thumb dimentions
				$image.stop().animate({
					'width'		: '100%',
					'height'	: '100%',
					'marginTop'	: '0px',
					...
					and animate the thumb postions and rotation
					 */
					if(ie)
						var param = {
			...
			//remove navigation
			removeNavigation();
			//if we are on single mode then disperse the thumb
			disperse();
		}
	});
	...
	$im_next.bind('click',function(){
		...
		if($next_thumb.length>0){
			var image_src	= $next_thumb.find('img').attr('src').replace('/thumb','/mid');
	...
	$im_prev.bind('click',function(){
		...
		if($prev_thumb.length>0){
			var image_src	= $prev_thumb.find('img').attr('src').replace('/thumb','/mid');
			var arr 		= Array.shuffle(positionsArray.slice(0));
			$thumb.each(function(i){
				var t = $(this);
				setTimeout(function(){
