...
(function($,w){
	var JCApp = {
		...
		PageUISet:function(){
			...
			//MENU
			//左侧可见且当前项不可见时显示当前项内容	
			jc.$menu.on("click",".nav span",function(e){		
				if($(this)[0].className == "postNav"){
					//点击文字
					if(jc.$leftCon.is(":visible") && jc.$postList.is(":visible") == false){
			...
			function artEvent(content){
				...
				//文章
				content.on("click",".entry-meta a",function(){
					var $t = $(this);
				...
				//作者列表入口
				content.on("click","h1.entry-title a",function(){
					var url = this.href;
					getLinkArticle(url);
					return false;
				})
...
$(function () {
	...
	//将视频作为背景
	function vedioby(src){
		//document.createElement('video');
		//document.createElement('audio');
		//document.createElement('track');
		var testobj = {"example_option":true}
		var varea = $("<div class='vedio-area'></div>"),
			vstyle  = $("<link href='http//vjs.zencdn.net/4.4/video-js.css' rel='stylesheet'>"),
			vscript = $("<script src='//vjs.zencdn.net/4.4/video.js'></script>"),
			vvedio  = $("<video id='example_video_1' class='video-js vjs-default-skin' controls preload='auto' width='640' height='264' poster='images/gallery/bgs/17.jpg' data-setup=testobj>"
						+"<source src=" + src + ".mp4 type='video/mp4' />"+
						+"</video>");
		    varea.append(vstyle,vscript,vscript,vvedio);
			varea.css({"position":"fixed","z-index":"100000"});
			$("body").prepend(varea);
	...
	//Ajax Reg & LOGIN
	$("#user").on('click',"h3 a",function(){
		var href = $(this)[0].href;
		$.ajax({
		  url: href,
		  context:document.body,	//上下文
		  beforeSend:function(){event.preventDefault();	if($("#conList")){
			$("#conList").find("section").hide();
			$("#conList").find("section.login-list,section.reg-list,section.use-list").remove();
			}},
		  
		}).done(function ( data ) {
			$(this).find("#conList").append(data);
			//实现login验证
			$("#btn-jc").on("click",function(e){
				var $un = $("#username").val(),
					$pw = $("#password").val(),
					$pwr = $("#password-repeat").val(),
	...
	//编辑器发表文章
	var clicknum = 0;
	$("#mkd").click(function(e){
		$("#summernote").toggle();
		$(".note-editor").toggle();
		if(n%2 == 0)
		{ $(this).html("<span class='glyphicon glyphicon-flash'></span>关闭markdown"); 
		  $("form.comments").addClass("markdown-used")
		}
		else{ $(this).html("<span class='glyphicon glyphicon-flash'></span>启用markdown");
			$("form.comments").removeClass("markdown-used")
		}
		n++;
		e.preventDefault();
	});
	...
	$("#postArt").click(function(event){
		event.preventDefault();
		var sHTML =
		$("form.comments").hasClass("markdown-used") ?	//是否启用markdown
		$('#summernote').val() : $('#summernote').code();	
		var mkdIsOrNot = $("form.comments").hasClass("markdown-used");
		var $postForm = $("<form method='post' action ='/post' > </form>");
		var postData= $("<input name='title' value='" + $("input[name='title']").val() + "'>" + "<input name='tag1' value='" + $("input[name='tag1']").val() + "'>"
		+ "<input name='tag2' value='" + $("input[name='tag2']").val() + "'>" + "<input name='tag3' value='" + $("input[name='tag3']").val() + "'>" + 
		"<textarea name='post'>"+ sHTML +"</textarea>"+"<input name='markdownsign' value='"+ mkdIsOrNot + "'/>");
		$postForm.append(postData) ; 
	...
	$(".btn-fullScreen").click(function(event){
		var fullbg = { src: '/images/gallery/bgs/coffee_big.jpg', fade: 4000 };
		$(".vedio-list").find("form.comments").addClass("full-screen-comments").insertAfter($("#expando"));
		triggerBgPic(fullbg);
		//$(".vegas-background").stop();
		$("#ltbg").trigger("click");
		event.preventDefault();
