...
function jsonOut($success,$info){
	if(!$carr){
		$carr = array ('success'=>$success,'info'=>$info);
	}
	...
	if($_POST['action'] == 'ajax_comment') {
		...
		if ( get_option('require_name_email') && !$user->ID ) {
			if ( 6> strlen($comment_author_email) || '' == $comment_author )
				...
				jsonOut(0,'Oopps,名字[Name]或邮箱[email]不对。');
			elseif ( !is_email($comment_author_email))
			//fail('Oopps,邮箱地址[Email]不对。');
			jsonOut(0,'Oopps,邮箱地址[Email]不对。');
		}
		if ( '' == $comment_content )
			//fail('是不是应该写点什么再提交？');
			jsonOut(0,'是不是应该写点什么再提交？');
		...
		//get_comment($comment, null);//这是我的调用评论函数，换成你的函数名。
		jsonOut(1,'评论成功!');
		die();
	}
}
